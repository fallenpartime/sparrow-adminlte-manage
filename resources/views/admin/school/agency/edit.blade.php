...
@section('wrapper_content')
            ...
            <div class="box-body">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>机构简介</label>
                            <textarea class="form-control" name="description" rows="3" placeholder="请输入机构简介">{{ $record->description }}</textarea>
                        </div>
                        <div class="form-group">
    ...
    <script>
        @if(!empty($record) && !empty($record->logo))
        initPictureList(uploadhandle, 'list-up', 'list_pic', '{{ $record->logo }}', '{{ $record->logo }}', 1);
        @endif
    </script>
    <script>
        function groupSave() {
