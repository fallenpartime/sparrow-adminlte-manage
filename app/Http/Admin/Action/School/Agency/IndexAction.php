...
use Admin\Services\Sql\School\Agency\AgencySqlProcessor;
...
class IndexAction extends BaseAction
{
    ...
    public function run()
    {
        ...
        $url = route(RouteConfig::ROUTE_AGENCY_LIST);
        list($page, $pageSize) = $this->getPageParams();
        $requestParams = $httpTool->getParams();
        list($model, $urlParams, $url) = (new AgencySqlProcessor())->getListSql(new CultivateAgency(), $requestParams, $url);
        $list = [];
        $total = $model->count();
        if ($total > 0) {
            $list = $this->pageModel($model, $page, $pageSize)->select(['id', 'no', 'name', 'logo', 'created_at'])->get();
        ...
        return $this->createView(ViewConfig::AGENCY_LIST, $result);
    }
    ...
    protected function initViewMenu()
    {
        $menuParams = [
            ...
            ['title' => RouteConfig::ROUTE_AGENCY_LIST, 'url' => 0, 'active' => 1],
        ];
    ...
    protected function processList($list)
    {
        foreach ($list as $key => $item) {
            $list[$key]->edit_url = route(RouteConfig::ROUTE_AGENCY_EDIT, ['id'=>$item->id]);
    ...
    protected function listAllowOperate($list, $key)
    {
        ...
        $authService = $this->getAuthService();
        if ($authService->isMaster || $authService->validateAction(RouteConfig::ROUTE_AGENCY_EDIT)) {
